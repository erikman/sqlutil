...
export class Table {
  ...
  _createTableSql() {
    ...
    let createIndexSql = '';
    if (indices) {
      createIndexSql = Object.getOwnPropertyNames(indices).map(indexName => {
    ...
    let sql = `CREATE TABLE ${this.schema.name} (\n${columnDescriptions.join(',\n')}\n);\n${createIndexSql}`;
    return sql;
  }
  ...
  _recreateTableIfNeeded() {
    let columnsInOld = () => {
      ...
      return this.db.all(sql).then(oldColumns => {
        ...
        return Object.keys(namedOldColumns);
      });
    };
    ...
    return Promise.join(columnsHaveChanged(), foreignKeysHaveChanged(),
      (recreate1, recreate2) => {
        ...
        if (recreate1 || recreate2) {
          return columnsInOld().then(namedOldColumns => {
            let copyColumns = _.intersection(Object.keys(this.schema.columns), namedOldColumns);

            console.log('namedOldColumns', namedOldColumns, copyColumns, Object.keys(this.schema.columns));

            let backupTableName = `backup_${this.schema.name}_${Date.now()}`;

            let sql1 = `BEGIN TRANSACTION; CREATE TEMPORARY TABLE ${backupTableName}(${namedOldColumns.join(',')});\nINSERT INTO ${backupTableName} SELECT ${namedOldColumns.join(',')} FROM ${this.schema.name};\nDROP TABLE ${this.schema.name};\nCOMMIT;\n`;
