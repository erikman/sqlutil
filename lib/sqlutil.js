...
export class SqlReadStream extends stream.Readable {
  /**
   * Extra options:
   * highWaterMark [int, 64]. The number of rows to cache.
...
export class SqlQuery {
  ...
  _buildSelectQuery() {
    ...
    let sql = `SELECT ${query.columns} ${query.from} ${query.where} ${query.groupBy} ${query.orderBy} ${query.limit};`;
    return {
      sql,
